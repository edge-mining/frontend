<template>
  <div class="border border-gray-200 bg-white px-5 pt-4 pb-3 sm:px-6 dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="flex justify-between gap-2 mb-4 sm:items-center">
      <div>
        <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Miner Control List</h3>
      </div>
      <div>
        <DropdownMenu :menu-items="menuControlListItems">
          <template #icon>
            <VerticalDots />
          </template>
        </DropdownMenu>
    ...
    <div class="max-w-full custom-scrollbar">
      <table class="w-full">
        <thead>
          <tr class="border-gray-100 border-y dark:border-gray-800">
            <th class="py-3 font-normal text-left">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">Type</p>
            </th>
            <th class="py-3 font-normal text-left">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">Miners</p>
            </th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
          ...
          <tr
            v-else-if="!isLoading && minerControls.length === 0"
            class="border-t border-gray-100 dark:border-gray-800"
          >
            <td colspan="6" class="py-3">
              <div class="flex items-center justify-center">
                <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                  Miner Controls not found.
                  <a
                    href="#"
                    class="text-brand-500 hover:text-brand-600"
                    @click.prevent="isMinerControlAddModal = true"
                  >
                    Add a miner control
                  </a>
          ...
          <tr
            v-else
            v-for="minerControl in minerControls"
            :key="minerControl.id"
          >
...
// Domain
import type { MinerControl, MinerStatus, MinerCreateSchema, MinerUpdateSchema } from '@/types/miner';
...
const props = defineProps<{
  minerControls: MinerControl[];
}>();

const minerStore = useMinerStore();
const isLoading = ref(false);
const error = ref<string | null>(null);
const isMinerControlAddModal = ref(false)
const isMinerControlUpdateModal = ref(false);
const menuControlListItems = [
  { label: 'Add a miner control', onClick: () => isMinerControlAddModal.value = true },
];
