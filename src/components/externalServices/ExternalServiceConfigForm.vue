...
const config = defineModel<ExternalServiceConfig>({ required: true });
...
const resolveRef = (refStr: string, schemaObj: ConfigSchema): any => {
  if (!refStr.startsWith("#/$defs/")) return null;
  const defName = refStr.replace("#/$defs/", "");
  return schemaObj.$defs?.[defName] || null;
};

// Get resolved property schema (handling $ref and anyOf)
const getPropertySchema = (property: any, schemaObj: ConfigSchema): any => {
  if (property.$ref) {
    return resolveRef(property.$ref, schemaObj);
  }
  if (property.anyOf) {
    const nonNullType = property.anyOf.find((item: any) => item.type !== "null");
    if (nonNullType) {
      if (nonNullType.$ref) {
        return resolveRef(nonNullType.$ref, schemaObj);
      }
...
const initializeDefaultValue = (property: any, schemaObj: ConfigSchema): any => {
  if (property.default !== undefined) {
    return property.default;
  }
  const resolvedProperty = getPropertySchema(property, schemaObj);
  ...
  if (resolvedProperty?.type === "object" && resolvedProperty?.properties) {
    ...
    Object.entries(resolvedProperty.properties).forEach(([key, prop]: [string, any]) => {
      obj[key] = initializeDefaultValue(prop, schemaObj);
    });
    return obj;
  }
...
watch(
  ...
  async (newAdapterType) => {
    ...
    loading.value = true;
    try {
      const cached = externalServiceStore.configSchemas.get(newAdapterType);
      if (cached) {
        schema.value = cached;
      } else {
        schema.value = await externalServiceStore.loadConfigSchema(newAdapterType);
      }
      // Initialize config with default values, preserving existing values
      if (schema.value?.properties) {
        const existingConfig = { ...config.value };
        const newConfig: ExternalServiceConfig = {} as any;
...
const getFieldType = (property: any, schemaObj: ConfigSchema) => {
  const resolvedProp = getPropertySchema(property, schemaObj);
