...
function flashTip(target: "monitor" | "external", original: string) {
  const tipRef = target === "monitor" ? energyMonitorTip : externalServiceIdTip;
  tipRef.value = "Copied!";
  window.setTimeout(() => {
...
<template>
  <tr>
        ...
        <div>
          <div class="text-xl flex items-center gap-1">
            <span
              ...
              :data-tip="energyMonitorTip ?? `ID: ${model.id}`"
            >
              <span
                role="button"
                tabindex="0"
                ...
                @click.stop="copyToClipboard(String(model.id)); flashTip('monitor', `ID: ${model.id}`)"
                @keydown.enter.stop.prevent="copyToClipboard(String(model.id)); flashTip('monitor', `ID: ${model.id}`)"
                @keydown.space.stop.prevent="copyToClipboard(String(model.id)); flashTip('monitor', `ID: ${model.id}`)"
              >
                <PhHash class="size-3" />
              </span>
            </span>
    ...
    <th>
      <div class="flex gap-2">
        ...
        <button class="btn btn-sm btn-error" @click="handleDeleteClick" title="Delete energy monitor"><PhTrash :size="15" /></button>
      </div>
    </th>
  </tr>
  ...
  <ConfirmDialog
    ...
    :message="`Are you sure you want to delete energy monitor '${model.name}'?`"
    confirm-text="Delete"
    variant="danger"
